...
vec3 render( in vec3 ro, in vec3 rd, in vec2 uv) {
  ...
  vec3 surfaceColor = .25 * mix(.1 * blue, orange * 2., (uv.y + .5) * getColor(uv * 0.1) * .05 * noise(vec3(getColor(fract(uv * 10.15) * time * .1))));

  float results = calcIntersection(ro, rd, uv);
  float t = results;

  vec3 lightPos = (vec3(0., .35 + sin(time * .1) * 0.500, 0.0));
  if (t > -.5) {
    ...
    r *= rotationMatrix(vec3(uv.x, uv.y, uv.x), time * .1 * PI * 2.);
    n += noise(vec3(getColor(fract(uv * 0.3) * time * .1)));
